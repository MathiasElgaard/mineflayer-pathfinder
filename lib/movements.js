...
class Movements {
  constructor (bot) {
    ...
    registry.blocksArray.map(x => Block.fromStateId(x.defaultState, 0)).forEach(block => {
    ...
    registry.blocksArray.forEach(block => {
      if (this.interactableBlocks.has(block.name) &&
          (block.name.toLowerCase().includes('door') ||
  ...
  getMoveJumpUp (node, dir, neighbors) {
    ...
    if (!blockC.physical && !blockC.openable) {
    ...
    if (this.canOpenDoors && blockB.openable) {
      const properties = blockB.getProperties()
      if (!properties.open) {
        toOpen.push({ x: node.x + dir.x, y: node.y + 1, z: node.z + dir.z, dx: 0, dy: 0, dz: 0 })
      }
    }
    else {
    ...
    neighbors.push(new Move(blockB.position.x, blockB.position.y, blockB.position.z, node.remainingBlocks - toPlace.length, cost, toBreak, toPlace, toOpen))
  }
  ...
  getMoveForward (node, dir, neighbors) {
    ...
    if (!(this.canOpenDoors && blockB.openable)) cost += this.safeOrBreak(blockB, toBreak)
    if (cost > 100) return

    // Open fence gates
    if (this.canOpenDoors && blockC.openable) {
      if (blockC.metadata & 2) {
        toOpen.push({ x: node.x + dir.x, y: node.y, z: node.z + dir.z, dx: 0, dy: 0, dz: 0 })
      }
    ...
    neighbors.push(new Move(blockC.position.x, blockC.position.y, blockC.position.z, node.remainingBlocks - toPlace.length, cost, toBreak, toPlace, toOpen))
  }
